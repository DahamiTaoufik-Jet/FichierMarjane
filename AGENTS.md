...
Memo d'architecture pour Codex. Lis ce fichier en premier avant toute modification du code.
...
- **Ne JAMAIS neutraliser un ancien script d'UI avec `enabled = false`.** Un composant desactive execute quand meme `Awake()` / `OnEnable()` — il s'abonne donc toujours aux evenements et peut reafficher son panneau — mais son `Update()` ne tourne plus, donc son minuteur de masquage ne s'execute jamais : le panneau reste a l'ecran indefiniment. Pour retirer un ecran migre, **supprimer le composant** (git sert de rollback) ou desactiver le GameObject **avant** le chargement de la scene, ce qui empeche `Awake()` de s'executer.
- **`FPSCanvas` est active/desactive au runtime** par `PlayerCamera` (bascule TPS/FPS). Un script pose dessus ne demarre donc qu'au premier passage en vue FPS, pas au chargement de la scene : un bug lie peut n'apparaitre qu'apres avoir appuye sur C. `InventoryPanelView`, `TextPuzzlePanelView` et `CrosshairController` y vivent.
